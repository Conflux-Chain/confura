...
- Cursor-paginated event log scans through `cfx_scanLogs` and `eth_scanLogs`, with forward or reverse traversal, exclusive resumable cursors, and optional pivot guards for detecting reorganizations between pages. Confura transparently spans indexed history and the not-yet-indexed full-node head while preserving page order. See the [feature overview](./doc/RPC_FEATURES.md#cursor-paginated-scanlogs) and [complete usage guide](./doc/PAGINATED_SCAN_LOGS.md).
...
- Support querying [early internal contract event logs](./doc/RPC_FEATURES.md#early-internal-contract-event-logs) through standard `cfx_getLogs` by reconstructing historical events from on-chain trace data.
